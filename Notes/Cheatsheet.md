...
> 经典例题：混合型随机变量的CDF
>
> 场景：投硬币决定坐火车还是汽车：硬币正面则火车，到达时间$X\sim\mathcal{U}(20,25)$；硬币反面则汽车，到达时间$X\sim\mathcal{U}(15,30)$。求$X$的CDF$F_X(x)$
> $$
...
| -------- | ----------------------- | ----------------------------------- | -------------------- | ---------------------- | ----------------------------------- |
| 伯努利   | $\mathrm{Ber}(p)$       | $p^k(1-p)^{1-k}$                    | $p$                  | $p(1-p)$               | 一次实验，成功为1                   |
| 二项     | $\mathrm{Bin}(n,p)$     | ${n\choose k}p^k(1-p)^{n-k}$        | $np$                 | $np(1-p)$              | $n$次独立$\mathrm{Ber}$实验成功次数 |
| 几何     | $\mathrm{Geo}(p)$       | $(1-p)^{k-1}p$                      | $\frac{1}{p}$        | $\frac{1-p}{p^2}$      | 第1次成功所需试验次数               |
